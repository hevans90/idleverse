...
const dashLineOptionsDefault: DashLineOptions = {
  dash: [10, 5],
  width: 1,
  color: 0xffffff,
  alpha: 1,
...
export class DashLine {
  ...
  private start: PIXI.Point | undefined;

  private dashSize: number;
  private dash: number[];

  private useTexture: boolean;
  private options: DashLineOptions = dashLineOptionsDefault;
   ...
   */
  constructor(graphics: PIXI.Graphics, options: Partial<DashLineOptions>) {
    this.options = { ...this.options, ...options };
    this.graphics = graphics;
    this.dash = this.options.dash;
    this.dashSize = this.dash.reduce((a, b) => a + b);
    this.useTexture = options.useTexture as boolean;
    this.setLineStyle();
  }
  ...
  lineTo(x: number, y: number, closePath?: boolean): this {
    ...
    if (this.useTexture) {
      ...
      const place =
        this.lineLength && this.lineLength % (this.dashSize * this.scale);
      let dashIndex = 0,
        dashStart = 0;
      let dashX = 0;
      for (let i = 0; i < this.dash.length; i++) {
        const dashSize = this.dash[i] * this.scale;
        if (place && place < dashX + dashSize) {
          dashIndex = i;
      ...
      while (remaining > 0) {
        ...
        if (closed) {
          const remainingDistance = DashLine.distance(
            x0 + cos * dist,
            y0 + sin * dist,
            this.start?.x as number,
            this.start?.y as number
          );
  ...
  closePath() {
    this.start && this.lineTo(this.start.x, this.start.y, true);
  }
  ...
  drawEllipse(
    ...
    const interval = (Math.PI * 2) / points;
    let first: { x: number; y: number } | undefined = undefined;
